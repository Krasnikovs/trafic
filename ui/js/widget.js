...
function deleteDashboard() {
	...
	if(conf) {
		...
		$.ajax({
			url: "/bwiot/api/v1/dashboard/delete/",
			type: "POST",
...
function dashboardCreater(id, name, desc, time) {
	var html = `<li class="nav-item dashboard-navbar" id="dashboard-id-${id}"><a class="nav-link" data-bs-toggle="collapse" data-bs-target=".navbar-collapse.show" href="javascript:getWidgets(${id},'${name}')">${name}</a></li></div>`;
	// $('ul.secnav li').removeClass("dashboard-navbar-active");
	// $("#mqtt-navbar").removeClass("dashboard-navbar-active");
...
function loadTopics() {
	...
	$.ajax({
		...
		success: function (result) {
			if(result["status"] == "Success") {
				var html = "";
				$("#topic-selector").find('option').remove();
				$("#topic-selector").append("<option selected disabled value=''>Select Topic</option>");
				html = ""
				for (var i = result.data.length - 1; i >= 0; i--) {
					html += "<option value="+ result.data[i] +">" + result.data[i] + "</option>";
				}
				$("#topic-selector").append(html);
			}
		}
	});
}
...
function loadKey() {
	var data = {};
	data["device"] = $("[list='device-selector']").val();
	data["topic"] = $("#topic-selector").val();// console.log(data);
	$.ajax({
		...
		success: function (result) {
			if(result["status"] == "Success") {
				var html = "";
				$("#cwidget-key-selector").find('option').remove();
				$("#cwidget-key-selector").append("<option selected disabled value=''>Select Key</option>");
				for (var i = result.data.length - 1; i >= 0; i--) {
					html += "<option value="+ result.data[i] +">" + result.data[i] + "</option>";
				}
				$("#cwidget-key-selector").append(html)
			}
		}
	});
}
...
function remove_max(){
	$("#widget_settings1").addClass('d-none');
	$('#top-navbar-2').removeClass('show');
	$('#deldash-btn').addClass('d-none');
	$('#dash-title').html('MQTTDashboard');
}

function getWidgets(id, dash_name) {

	$('#dash-title').html(dash_name);

	 if (window.matchMedia("(max-width: 767px)").matches){
	 	$("#widget_settings1").addClass('d-none');
	 }
	 else{
	 	$("#widget_settings1").removeClass('d-none');
	 }
	$('#deldash-btn').removeClass('d-none');
	$('#top-navbar-2').removeClass('show'); 
	...
	$('ul.secnav li').removeClass("dashboard-navbar-active");
	$("#dashboard-id-"+id).addClass("dashboard-navbar-active");
	var data = {};
	data["id"] = id;
	$.ajax({
		...
		success: function (result) {
			if(result["status"] == "Success") {
				...
				if(result.init.length > 0){

					$('#dash-status').addClass('d-none');
					...
					$('#dash-status').removeClass('d-none');
				}
				//createwidgetvisual()
...
function createWidget() {
	...
	if(data["type"]=="led"){
		...
		if((typeof(data["key"]) == "object" || Array.isArray(data["key"])) && data["key"].length > 1) {
			...
			keys = data["key"];
			data["data_key"][data["key"]] = {"sub_title" : data["sub_title"], "unit" : data["unit"], "color" : data["offset_color"]||"#ffffff"}
		}
	}
	var d_o = []
	data["widget_offset"] = {}
	...
	datum["data"] = {};
	datum["key"] = keys
	datum["data"] = JSON.stringify({"keys" : keys, "chrttype": data["chrttype"] ,  "key" : data["data_key"], "max" : data["maxval"], "min" : data["minval"], "offset" : data["offset"], "sub_topic" : data["sub_topic"], "act_msg" : data["act_msg"]||"Active", "inact_msg" : data["inact_msg"]||"Inactive", "yaxis_val" : data["yaxis_val"], "offset" : data["widget_offset"], "offset_data" : data["offset"], "act_color" : data["act_msg_color"], "inact_color" : data["inact_msg_color"]});
...
function initWidgetModal(th) {
	...
	switch($(th).val()) {
		...
		case "led" :
			...
			$("[name='offset_color']").attr("disabled", "true");
			try {		
				$("[name='act_msg_color']").removeAttr("disabled");
				$("[name='inact_msg_color']").removeAttr("disabled");
				}
			catch(err) {
				  console.log(err);
				}
		...
		case "switch" :
			...
			$("[name='unit']").attr("disabled", "true");
			$("[name='offset_color']").attr("disabled", "true");
			$("[name='act_msg_color']").attr("disabled", "true");
			$("[name='inact_msg_color']").attr("disabled", "true");
			$(".clokey").css("display","none");
			$("[name='key']").removeAttr("onkeydown");
			break;
	}
}
...
function createwidgetvisual(widget_id, title, device, topic, key, type, id) {
	//alert("create visual called");
		var html = 	"<section class='col-sm-12 col-md-6 col-lg-4 widget mt-2'>"
		html +=	"<div class='card card-1 widget_card' style='height:400px;'>"
		html += "<div class='card-header widget_header lato widget_card card-outline card-dark' data-toggle='tooltip' title='Grab Me!'><span class='text-center text-primary' style='font-size: 20px;'>"+ title +"</span></div>"
		html += "<div class='card-body lato' onload=''>"
		switch(type) {
			case "text" :
		...
		html += "</div><div class='card-footer lato mt-1'><i class='fa fa-clock-o'></i><span class='clock-text lato ml-2 text-info'></span><div class='ml-4 pull-right' style='font-size:1.2em'><span class='fa fa-times text-danger' data-toggle='tooltip' title='Delete Widget' onclick='deleteWidget(this, "+widget_id+")' style='cursor: pointer;'></span></div><div class='pull-right ml-4' style='font-size:1.2em'><span class='fa fa-pencil ml-2 text-primary editwid' data-toggle='tooltip' title='Edit Widget' onclick='editWidget(this, "+widget_id+")' style='cursor: pointer;'></span></div><div class='ml-4 pull-right' style='font-size:1.2em'><span class='fa fa-download text-success' data-toggle='tooltip' title='Download' device='"+device+"' topic='"+topic+"' key='"+key+"' onclick='DownloadData(this, "+widget_id+")' style='cursor: pointer;'></span></div></div></div></section>"
		$(html).insertBefore("#widget-creater");
...
function createwidgetvisualfull(widget_id, title, device, topic, key, type, id) {
	//alert("create visual  full 1 called");
		var html = 	"<section class='col-sm-12 col-md-6 col-lg-4 widget mt-2'>"
		html +=	"<div class='card card-1 widget_card' style='overflow:scroll'>"
		html += "<div class='card-header widget_card widget_header card-outline card-dark'><span class='text-center text-primary' style='font-size: 20px;'>"+ title +"</span></div>"
		...
		html += "</div><div class='card-footer lato mt-1'><i class='fa fa-clock-o'></i><span class='clock-text lato ml-1'></span><div class='ml-2 pull-right' style='font-size:1.2em'><span class='fa fa-download' data-toggle='tooltip' title='Download' device='"+device+"' topic='"+topic+"' key='"+key+"' onclick='DownloadData(this, "+widget_id+")' style='cursor: pointer;'></span> </div></div></div></section>"
		$("#widgets").append(html);
		switch(type) {
			case "text" :
				widgetValueInjecter(device, topic, id, key);
...
function createwidgetvisualfullwidget(widget_id, title, device, topic, key, type, id) {
	//alert("create visual full widget called");
		...
		html += "</div><div class='card-footer lato mt-1'><i class='fa fa-clock-o'></i><span class='clock-text lato ml-1'></span></div></div></section>"
				...
				if(topic in widget_list["text"][device]) {
					...
					for (var i = key.length - 1; i >= 0; i--) {
						...
						if(id == null) {
							...
							for(j in id) {
								$("#" + id[j]).parent().parent().parent().parent().find(".clock-text").html(time);
						...
						if(id == null) {
							...
							for(j in id) {
								...
								if(key[i] == "message-integer" || key[i] == "message-float" || key[i] == "message-string") {
									...
									$("#" +id).parent().parent().parent().css("background-color", bgcolr);
									$("#" +id).parent().parent().parent().parent().find(".card-header").find(".fa-times").removeClass("text-danger")
									$("#" +id).parent().parent().parent().parent().find(".card-header").find("span").removeClass("text-primary")
									$("#" +id).attr("style", "color: #" + colr);
									$("#" +id).parent().parent().parent().parent().css("color", "#" + colr);
...
function getWidgetId(type, device, topic, key) {
	widgets_id = "";
	//alert(JSON.stringify(widget_id));
...
$(function () {
  ...
  $('.connectedSortable').sortable({
    ...
    stop: function(event, ui) {
        ...
        $('.widget').each(function (index, id) {
        	var position = ui.item.offset();
        	//console.log($(this).find(".wid").attr("id"), index, id, $(this).position());
        	var card_top =  $(this).position().top;
        	var card_left = $(this).position().left;
        	var widget_change_id = $(this).find(".wid").attr("id");
        	var widget_index = index;
        	//widget_position[widget_change_id]["position"] = widget_index;
        	$(this).find(".wid").each(function() {
        		widget_position[$(this).attr("id")]["position"] = widget_index;
        	})
        	// console.log(widget_index, widget_change_id, index)
        });
	...
	$('.connectedSortable').bind('sortstart', function(event, ui) {
        $('.sort-highlight').append('<br><br><br><br><br><br><span class="text-secondary placetext text-center">Place Here <br><br><i class="fa fa-chevron-down text-center text-dark p_icon"></i></span>');
    });
	
});

function getSubTopic_widget() {
	var data = {};
	data["device_id"] = $("[list='device-selector']").val();
    $.ajax({
        method: "GET",
        data : data,
        ...
        success: function (result) {
        	if(result["status"] == "Success"){
				$("#widget-subtopic-selector").empty();
	            for (i = 0; i < result.active_subscription.length; i++) {
	                $('#widget-subtopic-selector').append(new Option(result.active_subscription[i]["topic"], result.active_subscription[i]["topic"], false, false));
	            }
        	}else{
