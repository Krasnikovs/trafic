		...
		success: function (result) {
			if(result["status"] == "Success") {
				...
				for(var i = 0; i< result["auth"].length; i++) {
					html += "<tr class='security-tr'><td>"+result["auth"][i]["username"]+"</td><td>"+result["auth"][i]["password"]+"</td><td>"+result["auth"][i]["access_type"]+"</td><td><i style=cursor:pointer class='fa fa-times text-danger' onclick='deleteAuth("+result["auth"][i]["id"]+")'></i></td></tr>";
				}
			}
